...
@st.cache_data
def load_data():
    df1 = pd.read_csv('output_with_embeddings_internwebben.csv')
...
# Streamlit UI
st.title("Fråga Falkenbergs Intranät")
...
if user_input:
    ...
    Du är en hjälpsam assistent som hjälper användaren att hitta information från Falkenbergs kommuns intranät. 
    Baserat på dokumenten som hittades, här är informationen för att hjälpa användaren:

    Dokument:
    {similar_texts.iloc[0]['document']}
    URL: {similar_texts.iloc[0]['url']}
    Likhetsscore: {similarities[0]}
    
    Dokument:
    {similar_texts.iloc[1]['document']}
    URL: {similar_texts.iloc[1]['url']}
    Likhetsscore: {similarities[1]}

    Dokument:
    {similar_texts.iloc[2]['document']}
    URL: {similar_texts.iloc[2]['url']}
    Likhetsscore: {similarities[2]}
    
    
    Hjälp användaren att få svar på sin fråga. Redovisa var du har fått informationen som du baserar ditt svar på, och hänvisa med länk till källan. Redovisa endast om dokumenten är relevant. Reply langague based on user input.
    """
